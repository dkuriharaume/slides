---
marp : true
theme: ume-orange
paginate: true
header : IS 対応マニュアル FS 引き継ぎ対応
footer : 2024.12.23 Mon マーケティング部 小松
transition : pull 300ms
---
<!-- _paginate : skip -->
<!-- _class : cover -->

# IS 対応マニュアル
## FS 引き継ぎ対応の方法

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 -->
<!-- _class: overview -->

<div>
<div>

## 目次

</div>
<div>

- [引継ぎタイミング](#timing)
- [見積希望の場合](#quote)
- [打ち合わせ希望の場合](#meeting)
- [その他の場合](#other)

</div>
</div>

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 -->
<span id="timing">

## FS引継ぎタイミング

FS引継ぎタイミング＝IS対応が終了するタイミング。

ISとFS兼任の場合、FSとしての対応がスタートするタイミングでもある。

- 見積希望
- 打ち合わせ希望
- システム連携希望：<strong>民泊INN / ホテルスマート / エアホストの場合のみ</strong>即前田Mgrへ引継ぎ。契約内容が煩雑のため。
- 価格交渉

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 -->
<span id=quote></span>

## 見積希望の場合

### 対応の流れ

1. プロパティ更新 - 問い合わせ内容詳細
1. 取引レコード作成
1. プロパティ更新 - この取引の概要
1. タスク作成
1. 関連付けの確認・操作
1. Trello更新

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

### 1.プロパティ更新 - 問い合わせ内容詳細

コンタクトレコードの下記プロパティを更新する。

- 問い合わせ内容分類（WF）の「見積もり」選択。
- 問い合わせパターン（WF）の「H - 引き継ぎのみ」選択。

すると自動でタスクが作成されることを確認。
タスクの説明欄通りに作業を進める。

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

### 2.取引レコード作成

コンタクトレコード表示画面の右側にある”取引（0件）”の右側「＋追加」クリック。

「取引を作成」画面が表示されるので、「取引名」を「会社名 姓さま LINKEY Plus (単体 or/and 遠隔)プラン 〇台 施工手配(あり or なし or 不明)」と入力。

> (ex.株式会社ユーエムイー 小松さま LINKEY Plus 遠隔プラン 1台 施工手配不明)

プラン名と台数と施工手配有無がわかるように記載する。

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

### 3.プロパティ更新 - この取引の概要

取引レコードの下記セクション内のプロパティを更新する。

- この取引の概要

各セクション・プロパティの入力ルールは[こちら](https://docs.google.com/document/d/1r7fNILlrSG4xram_D-H_yWUVuaJSOS5fATU5Q5cnhMk/edit?usp=sharing)

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

### 4.タスク作成

「見積作成依頼 to 営業事務」タスクを作成する。

自動化WFにて生成された”営業対応依頼_見積もり”タスクの説明欄通りにタスク処理。

#### 営業対応依頼_見積もりタスクの説明欄

[x] 取引レコードを作成
[x] 見積もり作成依頼 to 営業事務 を作成
[x] レコード担当者を営業に変更
[x] 依頼

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

<div class="mail" style="margin-bottom: 1rem">
<strong>補足 1</strong>

作成したタスクの説明欄にお客様へお送りしているメールの見積もり内容をそのままコピペ<br>
（【見積もり内容】◾︎プラン・台数...）
</div>

<div class="mail">
<strong>補足 2</strong>

スニペットを用いて引継ぎコメント追加。
各営業担当者と井出常務、栗原Ld、小松にそれぞれメンション。<br>

（例）@前田 聖也
お疲れ様です。小松です。
見積のご依頼をいただきました。
引継ぎますのでご対応よろしくお願いいたします。
@井出 俊樹 @栗原 大輔
お疲れ様です。小松です。
見積ご希望につき営業案件としております。

</div>

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 見積希望の場合-->

### 5.関連付けの確認・操作

コンタクトレコード～会社レコード～取引レコードの3レコードがそれぞれ正常に関連付けされているか確認する。
取引レコードのページにて会社レコードが関連付けされていないことが多いため、要チェック。

取引レコードで作成したタスクの関連付けが正常にされているか確認する。関連付けされていないコンタクト・会社・取引 があれば関連付けを行う。

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 打ち合わせ希望の場合-->
<span id="meeting"></span>

## 打ち合わせ希望の場合

### 対応の流れ

- プロパティ更新
- タスク作成・依頼

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 打ち合わせ希望の場合-->

### 1.プロパティ更新

コンタクトレコードの下記プロパティを更新する。
（担当者が変更になる場合のみ、コンタクト担当者を変更。）

- ライフサイクルステージを「SQL」に変更。
- 問い合わせ内容分類（WF）の「打ち合わせ」選択。
- 問い合わせパターン（WF）の「H - 引き継ぎのみ」選択。
- 追客完了日を選択。
- IS対応完了の理由の「打ち合わせパス」選択。
- その他ヒアリング事項など更新できる箇所がある場合は、随時更新。

自動でタスクが作成されることを確認。

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 打ち合わせ希望の場合-->

### 2.タスク作成

“打ち合わせ日時調整＆対応”タスクを作成する
自動化WFにて生成された”営業対応依頼_打ち合わせ”タスクの説明欄通りにタスク処理。

#### 営業対応依頼_打ち合わせタスクの説明欄

[ ] 打ち合わせ日時調整 & 対応タスクを作成
[ ] レコード担当者を営業に変更
[ ] ライフサイクルステージ = SQL に変更
[ ] 依頼

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > 打ち合わせ希望の場合-->

<div class="mail">
スニペットを用いて引継ぎコメント追加。<br>
各営業担当者と井出常務、栗原Ld、小松にそれぞれメンション。<br>
<br>
（例）<br>
@前田 聖也<br>
お疲れ様です。小松です。<br>
打ち合わせのご依頼をいただきました。<br>
引継ぎますのでご対応よろしくお願いいたします。<br>
@井出 俊樹 @栗原 大輔<br>
@小松 佳廉<br>
お疲れ様です。小松です。<br>
打ち合わせご希望につき営業案件としております。<br>
<br>
[ ] Trello ラベル更新<br>
[ ] IS 対応完了の理由 = 打ち合わせ引き継ぎを入力<br>
[ ] IS 対応完了日 = 本日の日付を入力<br>
</div>

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > その他の場合 -->
<span id="other"></span>

## その他の場合

その他価格交渉等、引継ぎが発生した場合

### 対応の流れ

- プロパティ更新
- タスク作成・依頼

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > その他の場合 -->

### 1.プロパティ更新

コンタクトプロパティ・会社プロパティを変更する

#### コンタクトプロパティ

- コンタクト担当者を営業担当者へ
- ライフサイクルステージ = SQL に変更
- IS 対応完了の理由 = 該当する値を選択
- IS 対応完了日 = 本日の日付を入力

#### 会社プロパティ

- 会社の担当者を営業担当者へ

---
<!-- _header : IS 対応マニュアル FS 引き継ぎ対応 > その他の場合 -->

<div class="columns-2">
<div>

### 2.タスク作成

タスクを作成する
こちら参照のうえ必要なタスクを随時作成し、営業担当者へ割り当て。
タスクのコメントより引継ぎコメント追加。
各営業担当者と井出常務、栗原Ld、小松にそれぞれメンション。

</div>

<div class="mail">
（例）<br>
<br>
@前田 聖也<br>
お疲れ様です。小松です。<br>
〇〇のご依頼をいただきました。<br>
引継ぎますのでご対応よろしくお願いいたします。<br>
<br>
@井出 俊樹 @栗原 大輔<br>
@小松 佳廉<br>
お疲れ様です。小松です。<br>
〇〇ご希望につき営業案件としております。
</div>
</div>

---
<!-- _class: cover -->

# おしまい

